<?php
    ...
    if (!empty($_POST)) {
        ...
        if ($isSuccess && $isUploadSuccess) {
            $db = Database::connect();
            $statement = $db->prepare("INSERT INTO items (name, description, price, category, brand, size, state, color, image) values(?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $statement->execute(array($name, $description, $price, $category, $brand, $size, $state, $color, $image));
            Database::disconnect();
