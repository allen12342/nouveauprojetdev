...
$nameError = $descriptionError = $priceError = $categoryError = $brandError = $colorError = $sizeError = $stateError = $imageError = $name = $description = $price = $category = $state = $brand = $color = $size = $image = "";
...
if (!empty($_POST)) {
    ...
    if (empty($brand)) {
        $brandError = 'Ce champ ne peut pas être vide';
        $isSuccess = false;
    }
    if (empty($size)) {
        $sizeError = 'Ce champ ne peut pas être vide';
        $isSuccess = false;
    }
    if (empty($color)) {
    ...
    if (empty($state)) {
        $stateError = 'Ce champ ne peut pas être vide';
        $isSuccess = false;
    }
    if (!empty($image)) {
        $isImageUpdated = true;
    ...
    if (($isSuccess && $isImageUpdated && $isUploadSuccess) || ($isSuccess && !$isImageUpdated)) {
        $db = Database::connect();
        if ($isImageUpdated) {
            $statement = $db->prepare("UPDATE items SET name = ?, description = ?, price = ?, category = ?, brand = ?, color = ?, size = ?, state = ?, image = ? WHERE id = ?");
            $statement->execute(array($name, $description, $price, $category, $brand, $color, $size, $state, $image, $id));
        } else {
            $statement = $db->prepare("UPDATE items SET name = ?, description = ?, price = ?, category = ?, brand = ?, color = ?, size = ?, state = ? WHERE id = ?");
            $statement->execute(array($name, $description, $price, $category, $brand, $color, $size, $state, $id));
        }
        Database::disconnect();
